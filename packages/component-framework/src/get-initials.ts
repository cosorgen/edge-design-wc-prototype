...
function cleanupDisplayName(displayName: string): string {
    ...
    output = output.replace(UNWANTED_ENCLOSURES_REGEX, "");
    output = output.replace(UNWANTED_CHARS_REGEX, "");
    output = output.replace(MULTIPLE_WHITESPACES_REGEX, " ");
    output = output.trim();
...
export function getInitials(
    ...
    const name = cleanupDisplayName(displayName);

    // For names containing CJK characters, and phone numbers, we don't display initials
    if (
        UNSUPPORTED_TEXT_REGEX.test(name) ||
        (!options?.allowPhoneInitials && PHONENUMBER_REGEX.test(name))
    ) {
        return "";
    }

    return getInitialsLatin(name, isRtl, options?.firstInitialOnly);
}
