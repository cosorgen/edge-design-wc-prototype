...
export default async (req, res) => {
    try {
        ...
        const metadata = await fetch(url)
            ...
            const title = titleElement.length > 0 ? titleElement.text() : 'Untitled';
            // Charset
            const charsetElement = $('meta[charset]');
            const charset = charsetElement.length > 0 ? charsetElement.attr('charset') : 'utf-8';
            // Description
            const descriptionElement = $('meta[name="description"], meta[property="og:description"]');
            const description = descriptionElement.length > 0
                ? descriptionElement.attr('content')
                : undefined;
            // Keywords
            const keywordsElement = $('meta[name="keywords"]');
            const keywords = keywordsElement.length > 0
                ? keywordsElement.attr('content')
                : undefined;
            // Image
            const imageElement = $('meta[property="og:image"]');
            let image = imageElement.length > 0 ? imageElement.attr('content') : undefined;
            if (image && image.startsWith('/')) {
                // Handle relative paths
                image = `${jsUrl.protocol}//${jsUrl.host}${image}`;
            }
            // Favicon
            const faviconElement = $('link[rel="icon"], link[rel="shortcut icon"], link[rel="apple-touch-icon"]');
            let favicon = faviconElement.length > 0 ? faviconElement.attr('href') : undefined;
        ...
        // Validate metadata
        if (metadata.image) {
            metadata.image = (await fetch(metadata.image, { method: 'HEAD' }).then((r) => r.ok))
                ? metadata.image
                : undefined;
        }
        if (metadata.favicon) {
        ...
        if (!cached)
            throwServerError('Failed to cache metadata', 500);
