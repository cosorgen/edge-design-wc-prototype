...
function GetRotatedHue(
  ...
  if (Object.keys(huesToRotations).length === 0) {
    throwServerError('huesToRotations is empty', 400);
  }
  if (sourceHue < 0.0 || sourceHue > 360.0) {
    throwServerError('sourceHue is out of range', 400);
  }
...
function GetAdjustedChroma(
  ...
  if (Object.keys(huesToChroma).length === 0) {
    throwServerError('huesToChroma is empty', 400);
  }
  if (sourceHue < 0.0 || sourceHue > 360.0) {
    throwServerError('sourceHue is out of range', 400);
  }
...
function GeneratePalette(seedColor: string, variant: SchemeVariant): Palette {
  ...
  switch (variant) {
    ...
    default:
      throwServerError('Invalid variant', 400);
  }

  return FromConfig(seedColor, config);
}

function TonesFromPalette(
  palette: Palette,
  steps = 101,
): Record<string, Record<number, string>> {
  const toneNumbers = [...Array(steps)].map((_, i) => i);
  return Object.fromEntries(
    Object.entries(palette).map(([key, tonalPalette]) => [
      key,
      Object.fromEntries(
        toneNumbers.map((tone) => [tone, hexFromArgb(tonalPalette.tone(tone))]),
      ),
    ]),
  );
}
...
export default async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!seed) {
      throwServerError('Missing required query parameters: seed', 400);
    }

    res.json({
      tonal: TonesFromPalette(GeneratePalette(seed as string, 'tonal')),
      vibrant: TonesFromPalette(GeneratePalette(seed as string, 'vibrant')),
      expressive: TonesFromPalette(
        GeneratePalette(seed as string, 'expressive'),
      ),
      neutral: TonesFromPalette(GeneratePalette(seed as string, 'neutral')),
    });
  } catch (err) {
    console.error(err);
