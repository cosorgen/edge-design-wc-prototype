...
const styles = css`
  ...
  #tabs {
    ...
    gap: ${paddingContentXxsmall};
    overflow: hidden;
    padding: max(10px, ${paddingContentXxsmall});
    margin: min(
      -10px,
      calc(0px - ${paddingContentXxsmall})
    ); /* for wings to not clip */
  }

  #window-grabber {
    flex: 1;
    height: calc(100% + (2 * ${paddingContentXxsmall}));
    margin-block-end: calc(0px - ${paddingContentXxsmall});
  ...
  mai-divider {
    ...
    margin-inline: calc(0px - (${paddingContentXxsmall} / 2));
  }
