...
export function applyChromiumTheme(
  ...
  seedColor: string,
  palette: 'neutral' | 'vibrant' | 'expressive' | 'tonal',
): Record<string, string> {
  // Generate mapping with real values from api
  const paletteColors = GeneratePaletteTones(
    GeneratePalette(seedColor, palette),
  );
  ...
  Object.entries(themeColors).forEach(([key, value]) => {
    ...
    if (cssVar) {
      ...
      console.warn(
        `Token ${key} not found in smtcTokens, theme color not applied.`,
      );
    }
  });
